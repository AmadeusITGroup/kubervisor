...
import (
	...
	"github.com/amadeusitgroup/kubervisor/examples/demo/pkg/comparator"
...
func run() error {
	...
	cfg := comparator.NewConfig(promRegistry)
	cfg.AddFlags(pflag.CommandLine)
	pflag.CommandLine.AddGoFlagSet(goflag.CommandLine)
	pflag.Parse()
	goflag.CommandLine.Parse([]string{})

	fmt.Println("Providers:", cfg.Providers)

	comparatorSvc := comparator.NewService(cfg)

	r := mux.NewRouter()
	r.HandleFunc("/api/v1/search", comparatorSvc.SearchHandler).Methods("GET")
