...
func (ctrl *Controller) onAddKubervisorService(obj interface{}) {
	bc, ok := obj.(*api.KubervisorService)
	if !ok {
	...
	if !apiequality.Semantic.DeepEqual(bc.Status, api.KubervisorServiceStatus{}) {
...
func (ctrl *Controller) onDeleteKubervisorService(obj interface{}) {
	...
	if !ok {
		...
		bc, ok = tombstone.Obj.(*api.KubervisorService)
		if !ok {
...
func (ctrl *Controller) onUpdateKubervisorService(oldObj, newObj interface{}) {
	bc, ok := newObj.(*api.KubervisorService)
	if !ok {
